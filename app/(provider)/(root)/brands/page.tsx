...
import ballangAPI, { Brand, Product } from "@/api/ballangAPI";
...
export default function BrandsDetailPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const response1 = await ballangAPI.get("/brands");
        const data1 = await response1.data;

        const response2 = await ballangAPI.get("/products");
        const data2 = await response2.data;

        console.log(data1);
        console.log(data2);

        setBrands(data1.result);
        setValue(data2.result);
      } catch (e) {
        console.log(e);
      }
    })();
